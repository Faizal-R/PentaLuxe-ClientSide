...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    if (variant && product.DiscountPercentage) {
      const finalPrice = variant.price - (variant.price * product.DiscountPercentage / 100);
      ...
      <div className="relative z-10 w-full px-6 lg:px-12 py-8">
        ...
        <div className="flex justify-between items-center mb-12">
           ...
           >
             ...
             <span className="uppercase text-[9px] tracking-[0.5em] font-bold">Return to Collection</span>
           </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-12 lg:gap-20 items-start max-w-7xl mx-auto">
          ...
          <div className="lg:sticky lg:top-24 space-y-6">
             <div className="relative aspect-square bg-gradient-to-br from-[#0c1110] to-black border border-white/5 rounded-[40px] overflow-hidden group shadow-2xl flex items-center justify-center">
                ...
                >
                  <Heart className={`w-4 h-4 mx-auto ${wishlistToggle ? "fill-current" : ""}`} />
                </button>
             </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                 <div className="flex items-center gap-2">
                    ...
                    <span className="text-emerald-500 tracking-[0.3em] uppercase text-[7px] font-bold">Profile: {product.CategoryId.categoryName}</span>
                 </div>
                 {/* In Stock Indicator */}
                 <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                    <div className="w-1 h-1 bg-emerald-500 rounded-full animate-pulse" />
                    <span className="text-[7px] text-emerald-500 font-bold uppercase tracking-wider">In Stock</span>
                 </div>
              </div>
              
              <div className="space-y-0.5">
                 <h1 className="text-2xl md:text-3xl font-serif text-white tracking-tighter leading-tight">{product.Name}</h1>
                 <div className="flex items-center gap-2 pt-0.5">
                    <div className="flex gap-0.5">
                      {[1,2,3,4,5].map(s => <Star key={s} className="w-2.5 h-2.5 fill-emerald-500 text-emerald-500" />)}
                    </div>
                    <span className="text-[8px] uppercase tracking-widest text-slate-600 font-bold border-l border-white/5 pl-2 ml-1">Archive #842</span>
                 </div>
              </div>
            ...
            <div className="space-y-5 pt-2">
               ...
               <div className="flex items-center justify-between border-b border-white/5 pb-3">
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl md:text-4xl font-mono font-bold text-white tracking-tighter">₹{discountPrice}</span>
                    {product.DiscountPercentage > 0 && (
                      <span className="text-base text-slate-700 line-through font-mono">₹{selectedVariant?.price}</span>
                    )}
                  </div>
               ...
               <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] uppercase tracking-[0.2em] text-emerald-500/60 font-bold">Extraction Protocol</span>
                    <span className="text-[8px] uppercase tracking-widest text-white/10 font-mono italic">{selectedVolume}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {product.Variants.map((v) => (
                      ...
                      >
                        <span className="text-[11px] font-bold font-mono leading-none">{v.volume.replace(/\D/g,'')}</span>
                        <span className="text-[10px] font-bold opacity-40 mt-0.5 text-center">ML</span>
                      </button>
                    ))}
                  </div>
               </div>
               ...
               <div className="space-y-4">
                 ...
                 <div className="flex items-center justify-center gap-6 opacity-20">
                    <div className="flex items-center gap-1.5">
                       ...
                       <span className="text-[7px] uppercase tracking-widest font-bold">Priority Shipping</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                       <ShieldCheck className="w-2.5 h-2.5 text-emerald-500" />
                       <span className="text-[7px] uppercase tracking-widest font-bold">Essence Grade A</span>
                    </div>
                 </div>
               </div>
            </div>
        ...
        <section className="mt-32 pt-24 border-t border-emerald-500/10 grid grid-cols-1 md:grid-cols-3 gap-16">
           {[
             { title: "Scent Evolution", desc: "A complex molecular structure that shifts from metallic top notes to a deep obsidian resonance over 14 hours." },
             { title: "Studio Process", desc: "Cold-pressed in controlled environments to preserve the delicate chains of olfactory data within every decant." },
             { title: "Preservation", desc: "Housed in heavy obsidian glass to eliminate UV interference and maintain the integrity of the essence." }
           ].map((item, i) => (
             <div key={i} className="space-y-4 group">
               <h4 className="text-[10px] uppercase tracking-[0.5em] text-emerald-500 font-bold group-hover:translate-x-2 transition-transform duration-500">{item.title}</h4>
        ...
        <section className="mt-32 mb-16 space-y-16">
           <div className="flex items-baseline gap-12">
              <h2 className="text-5xl md:text-7xl font-serif text-white tracking-tighter">Parallel <span className="text-emerald-500 italic underline decoration-1 underline-offset-8">Visions.</span></h2>
              <div className="hidden md:block h-[1px] flex-grow bg-emerald-500/10" />
           </div>
           
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {relatedProducts?.slice(0, 4).map((product) => (
                <div key={product._id} className="hover:-translate-y-4 transition-transform duration-700">
