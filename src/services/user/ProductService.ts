...
export const ProductService = {
  ...
  getRelatedProducts: async (categoryName: string, productId: string) => {
    try {
      const res = await api.get(
        USER_API_ROUTES.PRODUCTS.GET_RELATED_PRODUCTS(categoryName, productId),
      );
      return res.data;
    } catch (error) {
      const parsedError = parseAxiosError(
  ...
  searchByCategory: async (
    ...
    try {
      const res = await api.get(USER_API_ROUTES.PRODUCTS.SEARCH_BY_CATEGORY, {
        params,
      });
      return res.data;
    } catch (error) {
  ...
  getProductsByCategory: async (categoryId: string) => {
    try {
      const res = await api.get(
        USER_API_ROUTES.CATEGORIES.GET_ALL_PRODUCTS_BY_CATEGORY(categoryId),
      );
      return res.data;
    } catch (error) {
      const parsedError = parseAxiosError(
